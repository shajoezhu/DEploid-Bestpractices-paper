...
\begin{enumerate}
  ...
  \item In practice, we estiamte the within sample relatedness using the identity-by-descent (IBD) state $\mathcal{S}$ between the mixed strains. Let's recall the notations from \citet{Zhu2019}, and use a Bayesian approach and define the posterior probabilities of $K$, $\mathbf{w}$, $\mathbf{h}$ and $\mathcal{S}$ as the following:
  \begin{equation*}
...
\end{equation*}
  ...
  This allows us to be flexible with inference process orders. In practice, certain executation orders perform better than the others:
  \begin{enumerate}
