...
Therefore, a mixed infection can be completely described by the number of co-existing strains $k$, their relative proportions $p$, within sample relatedness $r$ and their associated haplotypes $h$.
...
\begin{enumerate}
  \item Reference panel not only essential for haplotyle quality, also improves the inference of k.
  \item Given the correct k, Ibd improves the proportion inference, as a result, it also improves haplotype inference
  \item It does not require whole genome to infer k. A few hundred of snp would do the job
